...
public class Maths {
    ...
    public static IntegralOperation TestCalc() {
        //simulate explosion
        double a = 1;//potential energy start (symetrical)
        double e = 4;//4.564767917455;//compacted mass(TL)->4.564767917455 chaos? 0.29 Result 4.000000000000003 << 	0=-3.141592653589793
        final double b = 100;//half calc decay(100)
        double normal = 0;//normalizing constant(fire) chaos if negative -e
        final double dim = 1.0;//4.0 sphere 2.0 circle 1.0 point
        NumberSequance ns = new NumberSequance(new Double[]{0.0});
        IntegralOperation in = new IntegralOperation(a, b + 0.2, normal);
        in.SetVariables(ns);
        in.OperateUsing((x) -> (1.0 / x - x * (1.0 - 1.0 / b)));//scale function
        in.iterate = (x) -> (x) / b;//scaler
        //in.resultant=(x)->e/(x*dim/3.0);//ratio scale (b/3)*0.5 (using inverse for explosion)//-1.4531450450773544	
        in.resultant = (x) -> e / (x * dim / 3.0);//ratio scale (b/3)*0.5 (using inverse for explosion)
        //
        int i = 0;
        double at = 0;
        double from = 0;
    ...
    NumberSequance Shuffle(int times) {
        ...
        for (int t = 0; t < times; t++) {
            ...
            switch (i) {
                ...
                case 6: {
                    even.Reverse();
                    result = odd.Inserted(even, Random.Next(odd.length));//cut

                    break;
                }
                case 7: {
                    odd.Reverse();
                    result = even.Inserted(odd, Random.Next(even.length));//cut
                ...
                case 10: {
                    result = odd.Inserted(even, Random.Next(odd.length)).Reversed();//cut

                    break;
                }
                case 11: {
                    result = even.Inserted(odd, Random.Next(even.length)).Reversed();//cut
                ...
                case 18: {
                    even.Reverse();
                    result = odd.Intergrated(even, Random.Next(odd.length));//cut

                    break;
                }
                case 19: {
                    odd.Reverse();
                    result = even.Intergrated(odd, Random.Next(even.length));//cut
                ...
                case 22: {
                    result = odd.Intergrated(even, Random.Next(odd.length)).Reversed();//cut

                    break;
                }
                case 23: {
                    result = even.Intergrated(odd, Random.Next(even.length)).Reversed();//cut
        ...
        switch (i) {
            ...
            case 2: {
                result = even.Inserted(odd.Reversed(), Random.Next(even.length));//cut
                break;
            }
            case 3: {
                result = odd.Inserted(even.Reversed(), Random.Next(odd.length));//cut
                break;
            }
            case 4: {
                result = even.Inserted(odd, Random.Next(even.length));//cut
        ...
        return even.Inserted(odd, Random.Next(even.length));//final cut
    }
...
class IntegralOperation extends RingList<NumberSequance> {
    ...
    OneStateFunction intergrate = (x) -> 0.5 * x * x;
