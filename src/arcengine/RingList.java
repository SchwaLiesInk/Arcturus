...
public class RingList<T> implements java.util.Collection<T> {
    ...
    public T Remove(T obj) {
        ...
        Node<T> val = Start();
        for (; val.data != null; val = val.next) {
            if (val.data == obj) {
                Node<T> swap = val.next;
                val.prev.next = swap;
                swap.prev = val.prev;
                length--;
                break;
    ...
    public void CopyToArray(Node<T>[] array) {
        ...
        if (length > 0) {
            int i = 0;
            for (Node<T> n = Start(); n.data != null && i < array.length; n = n.next) {
                array[i] = new Node();
                array[i].data = n.data;
                i++;
            }
    ...
    public boolean containsAll(Collection c) {
        return c.stream().noneMatch((o) -> ((Contains((T) o) < 0)));
    }
